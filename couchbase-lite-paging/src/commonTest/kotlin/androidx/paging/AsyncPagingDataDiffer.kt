...
constructor(
     ...
     * false.
     */
    private val childLoadStateListeners = ConcurrentMutableList<(CombinedLoadStates) -> Unit>()
    ...
    fun addLoadStateListener(listener: (CombinedLoadStates) -> Unit) {
        if (parentLoadStateListener.load() == null) {
    ...
    fun removeLoadStateListener(listener: (CombinedLoadStates) -> Unit) {
        ...
        if (childLoadStateListeners.isEmpty()) {
            val parent = parentLoadStateListener.load()
    ...
    internal fun addLoadStateListenerInternal(listener: (CombinedLoadStates) -> Unit) {
        parentLoadStateListener.store(listener)
    ...
    internal val internalLoadStateListener: (CombinedLoadStates) -> Unit = { loadState ->
        if (!inGetItem.value) {
            ...
            LoadStateListenerJob?.cancel()
            // enqueue child listeners
            LoadStateListenerRunnable.loadState.store(loadState)
            @OptIn(DelicateCoroutinesApi::class)
            LoadStateListenerJob = GlobalScope.launch(LoadStateListenerDispatcher) { LoadStateListenerRunnable.run() }
        }
    }

    private val LoadStateListenerDispatcher = mainDispatcher
    private var LoadStateListenerJob: Job? = null
    ...
    private val LoadStateListenerRunnable =
        object : Runnable {
            ...
            override fun run() {
                loadState.load()?.let { state -> childLoadStateListeners.forEach { it(state) } }
            }
        }
}
